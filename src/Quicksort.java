...
public class Quicksort {
    ...
    public static void main(String[] args) throws Exception {
        if (args.length == 3) {
            ...
            int numOfBuffers = Integer.valueOf(args[1]);
            String statFile = args[2];

            BufferPool buffPool = new BufferPool(inputFile, numOfBuffers);
            QSort qSort = new QSort(buffPool, inputFile, statFile);
        }
        else {
            System.out.println("Found wrong number of command arguments: "
                + args.length);
        }
    }
}
